...
- [x] Testeo manual del usuario: `COMPOSE_PROFILES=queues` levanta rabbitmq +
      worker (healthy), worker consume `ingest`/`enrich` sin errores, sin
      tocar backend/database/frontend/scheduler ya corriendo.
- [x] Merge a `main` con el upgrade probado.
...
## 5. Hacia v2.0 (promovido al roadmap)

> Los items de v2.0 que originalmente vivían acá se promovieron a epics de
> primera clase en `docs/roadmap/`:
>
> - Flags en instalación / scaffolding → `docs/roadmap/epics/A-install-time-flags.md`
> - Propagación de upgrades a forks → `docs/roadmap/epics/B-fork-upgrade-model.md`
> - Worker de producción → `docs/roadmap/epics/C-prod-worker.md`
>
> El roadmap general y la cadencia de versiones están en
> `docs/roadmap/README.md`. Este documento queda como **registro histórico** del
> trabajo de v1.0 (profiles).
